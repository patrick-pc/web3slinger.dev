  ...
  var methods = {
        ...
        p.commits.forEach(function (d, i) {
          ...
          if (i < 2) {
            ...
            d.committerGravatar = Mustache.render(
              '<img class="gha-gravatar-commit" src="https://avatars.githubusercontent.com/u/93359354" width="16" />',
    ...
    getOutputFromRequest: function (url, callback) {
      ...
      if (
        ...
        config.credentials.personalAccessToken
      ) {
        request.setRequestHeader(
          'Authorization',
          'Basic ' +
            btoa(config.credentials.username + ':' + config.credentials.personalAccessToken)
        )
      }
...
var templates = {
    ...
    PullRequestEvent:
      '{{eventType}} pull request {{{pullRequestLink}}}<br>{{{userGravatar}}}<small>{{payload.pull_request.title}}</small>{{{mergeMessage}}}',
    PullRequestReviewEvent:
      '{{eventType}} pull request {{{pullRequestLink}}}.<br>{{{userGravatar}}}<small>{{payload.review.body}}</small>',
